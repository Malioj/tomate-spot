...
// Función para obtener fechas + opción "Otra fecha"
const getDateOptions = (): DateOption[] => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    // Solo mostramos días abiertos (Mié-Dom)
...
export default function ReservasPage() {
  const TELEFONO_BAR = "5493531234567"; 
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const fechaBonita = fechaObj ? fechaObj.label : formData.fecha;

    // AQUI ESTA LA MAGIA: Agregamos asteriscos (*) para la negrita
    let texto = `Hola Tomate Spot! Quiero reservar mesa:\n\n`;
    texto += `*Nombre:* ${formData.nombre}\n`;
    texto += `*Día:* ${fechaBonita}\n`;
    texto += `*Hora:* ${formData.hora} hs\n`;
    texto += `*Personas:* ${formData.personas}`;
    
    // Usamos encodeURIComponent para que WhatsApp entienda los asteriscos
          ...
          <div>
            ...
            {showCalendar && (
              <div className="mt-4 animate-fadeIn">
                <label className="text-xs text-red-500 mb-1 block">Selecciona una fecha específica:</label>
                <input 
                  type="date" 
                  name="fecha"
